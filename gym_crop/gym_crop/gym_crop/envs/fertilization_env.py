...
class FertilizationEnv(gym.Env):
    ...
    def step(self, action):
        ...
        ob, reward, episode_over, info : tuple
            ob (object) :
                ...
                the environment.
                In order, the obtained variables are:
                - LAI: Leaf Area Index, the leaf area per unit of soil area (m² leaf/m² soil)
                - WLVD: Weight of dead leaves (based on context from figures showing biomass partitioning)
                - WLVG: Weight of green leaves (mentioned in biomass partitioning discussions)
                - WRT: Weight of roots (discussed in biomass partitioning sections)
                - WSO: Weight of storage organs (used in biomass partitioning discussions)
                - WST: Weight of stems (shown in biomass partitioning figures)
                - TAGBM: Appears to represent Total Above Ground BioMass (though not explicitly defined in paper)
                - TGROWT: Total Growth rate - likely represents overall crop growth over time
                - NUPTT: Nitrogen UPTake Total - likely the cumulative nitrogen uptake by the plant
                - TRAN: TRANspiration - likely represents actual transpiration rate of the crop
                - TIRRIG: Total IRRIGation - cumulative irrigation water applied
                - TNSOIL: Total Nitrogen in SOIL - probably represents available nitrogen in soil
                - TRAIN: Total RAINfall - likely cumulative rainfall amount
                - TRANRF: TRANspiration Reduction Factor - appears to be a water stress factor affecting transpiration (mentioned in Fig. 1 as "RFtr")
                - TRUNOF: Total RUNOFF - likely represents cumulative water runoff from the field
                - TTRAN: Total TRANspiration - probably cumulative transpiration over the growing period
                - WC: Water Content - likely represents soil water content
                - Weather data [..]
            reward (float) :
    ...
    def _get_weatherdataprovider(self):
        ...
        return NASAPowerWeatherDataProvider(*location)
